...
from apps.contrib.tests.mixins import OauthTestCase
...
class AuthAPITests(OauthTestCase):
    ...
    def make_register_data(self):
        user = self.make_user(commit=False)
        data = dict()
    ...
    def test_register(self):
        """
        Test: /api/credentials/register/ with MOBILE Client
        """
        request, user, expected = self.make_register_data()
        response = self.client.post(reverse('credentials:register'), request, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)  # Disponibility
        self.assertEqual(User.objects.filter(username=user.username).count(), 1)  # Atomicity
        self.assertEqual(response.data["username"], expected["username"])  # Validity
        self.assertEqual(response.data["email"], expected["email"])
    ...
    #     mail.outbox = []
    #
    #     response = self.client.post(path=url,data={"email_or_user": self.user.username})
    #
    #     self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    #     self.assertEqual(len(mail.outbox), 1)
    #
